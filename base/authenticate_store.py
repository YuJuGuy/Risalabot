...
def authstore(request):
    """
    Authentication flow for Salla with proper state parameter.
    The state parameter helps prevent CSRF attacks.
    """
    # Generate a secure state parameter that's at least 8 characters
    state = get_random_string(length=32)  # Using Django's get_random_string for security
    #https://accounts.salla.sa/oauth2/auth?client_id=2ab59beb-20a7-4ea6-a491-7c2c1a2673e0&response_type=code&redirect_uri=https%3A%2F%2Faccounts.salla.sa%2Fcallback%2F2138798897&scope=settings.read&state=12321312
    
    auth_url = (
        'https://s.salla.sa/apps/install/2138798897'
    )
